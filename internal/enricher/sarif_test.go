...
func TestBuildRuleIndex(t *testing.T) {
	t.Run("parses rules from tool JSON", func(t *testing.T) {
		...
		index, ruleIDs := buildRuleIndex(toolJSON)
		...
		// Verify positional slice.
		require.Len(t, ruleIDs, 2)
		assert.Equal(t, "CVE-2024-0001", ruleIDs[0])
		assert.Equal(t, "CVE-2024-0002", ruleIDs[1])
	})

	t.Run("returns nil for empty tool", func(t *testing.T) {
		index, ruleIDs := buildRuleIndex(nil)
		assert.Nil(t, index, "expected nil index for nil tool")
		assert.Nil(t, ruleIDs, "expected nil ruleIDs for nil tool")
	})

	t.Run("returns nil for tool without rules", func(t *testing.T) {
		index, ruleIDs := buildRuleIndex(json.RawMessage(`{"driver":{"name":"Trivy"}}`))
		assert.Nil(t, index, "expected nil index for tool without rules")
		assert.Nil(t, ruleIDs, "expected nil ruleIDs for tool without rules")
	})

	t.Run("returns nil for invalid JSON", func(t *testing.T) {
		index, ruleIDs := buildRuleIndex(json.RawMessage(`not json`))
		assert.Nil(t, index, "expected nil index for invalid JSON")
		assert.Nil(t, ruleIDs, "expected nil ruleIDs for invalid JSON")
	})
}
...
func TestEnrichSARIF_FallsBackToRuleIndex(t *testing.T) {
	epssSource := setupEPSSSource(t)
	kevSource := setupKEVSource(t)
	enricher := New(epssSource, kevSource)

	// Result has empty ruleId but ruleIndex=0, which points to CVE-2024-1234
	// in the rules array.
	report := makeSARIFReportWithTool(
		toolJSONWithRules(),
		types.SARIFResult{
			...
			RuleIndex: intPtr(0),
			Level:     "error",
			Message:   json.RawMessage(`{"text":"Resolved via ruleIndex"}`),
		},
	)
	...
	// Verify enrichment used CVE-2024-1234 (from ruleIndex fallback).
	r0 := results[0]
	require.NotNil(t, r0.Properties, "Properties is nil")
	vpRaw, ok := r0.Properties["vulnPrio"]
	require.True(t, ok, "vulnPrio missing from Properties")

	var vp types.VulnPrio
	require.NoError(t, json.Unmarshal(vpRaw, &vp), "unmarshal VulnPrio")
	require.NotNil(t, vp.EPSS, "EPSS is nil")
	require.NotNil(t, vp.EPSS.Score, "EPSS.Score is nil")
	assert.InEpsilon(t, 0.97, *vp.EPSS.Score, 0.01, "EPSS.Score should match CVE-2024-1234")
	require.NotNil(t, vp.KEV, "KEV is nil")
	assert.True(t, vp.KEV.Listed, "KEV.Listed should be true for CVE-2024-1234")
}

func TestEnrichSARIF_RuleIdTakesPrecedence(t *testing.T) {
	epssSource := setupEPSSSource(t)
	kevSource := setupKEVSource(t)
	enricher := New(epssSource, kevSource)

	// Result has both ruleId=CVE-2024-1234 and ruleIndex=1 (which points to
	// CVE-2023-5678). The ruleId should take precedence.
	report := makeSARIFReportWithTool(
		toolJSONWithRules(),
		types.SARIFResult{
			RuleID:    "CVE-2024-1234",
			RuleIndex: intPtr(1),
			Level:     "error",
			Message:   json.RawMessage(`{"text":"Both ruleId and ruleIndex present"}`),
		},
	)
	...
	// Verify enrichment used CVE-2024-1234 (from ruleId, not ruleIndex).
	r0 := results[0]
	require.NotNil(t, r0.Properties, "Properties is nil")
	vpRaw, ok := r0.Properties["vulnPrio"]
	require.True(t, ok, "vulnPrio missing from Properties")
	...
	require.NotNil(t, vp.EPSS, "EPSS is nil")
	require.NotNil(t, vp.EPSS.Score, "EPSS.Score is nil")
	assert.InEpsilon(t, 0.97, *vp.EPSS.Score, 0.01, "EPSS.Score should match CVE-2024-1234 (ruleId takes precedence)")
	require.NotNil(t, vp.KEV, "KEV is nil")
	assert.True(t, vp.KEV.Listed, "KEV.Listed should be true for CVE-2024-1234")
}

func TestEnrichSARIF_InvalidRuleIndex(t *testing.T) {
	epssSource := setupEPSSSource(t)
	kevSource := setupKEVSource(t)
	enricher := New(epssSource, kevSource)

	t.Run("out of range positive index", func(t *testing.T) {
		report := makeSARIFReportWithTool(
			toolJSONWithRules(),
			types.SARIFResult{
				RuleID:    "",
				RuleIndex: intPtr(999),
				Level:     "warning",
				Message:   json.RawMessage(`{"text":"Out of range ruleIndex"}`),
			},
		)
		...
		// With invalid ruleIndex and empty ruleId, cveID remains empty.
		// Enrichment should still succeed with no EPSS/KEV match.
		r0 := results[0]
		require.NotNil(t, r0.Properties, "Properties is nil")
		vpRaw, ok := r0.Properties["vulnPrio"]
		require.True(t, ok, "vulnPrio missing from Properties")
		...
		require.NoError(t, json.Unmarshal(vpRaw, &vp), "unmarshal VulnPrio")
		require.NotNil(t, vp.EPSS, "EPSS is nil")
		assert.Nil(t, vp.EPSS.Score, "EPSS.Score should be nil for unresolved CVE")
		require.NotNil(t, vp.KEV, "KEV is nil")
		assert.False(t, vp.KEV.Listed, "KEV.Listed should be false for unresolved CVE")
	})
	...
	t.Run("negative index", func(t *testing.T) {
		report := makeSARIFReportWithTool(
			...
			types.SARIFResult{
				...
				RuleIndex: intPtr(-1),
				Level:     "warning",
				Message:   json.RawMessage(`{"text":"Negative ruleIndex"}`),
			},
		)
	...
	t.Run("nil ruleIndex with empty ruleId", func(t *testing.T) {
		report := makeSARIFReportWithTool(
			toolJSONWithRules(),
			types.SARIFResult{
				RuleID:  "",
				Level:   "warning",
				Message: json.RawMessage(`{"text":"No ruleId and no ruleIndex"}`),
			},
		)
