...
// WithHTTPClient sets a custom HTTP client for the source. Returns the source
// for method chaining. If c is nil, the default client is retained.
...
func (s *Source) download() ([]byte, error) {
	now := time.Now().UTC()
	today := now.Format("2006-01-02")
	yesterday := now.AddDate(0, 0, -1).Format("2006-01-02")

	data, err := s.downloadForDate(today)
	if err == nil {
		return data, nil
	}

	data, err2 := s.downloadForDate(yesterday)
	if err2 == nil {
		return data, nil
	}
...
func (s *Source) downloadForDate(date string) ([]byte, error) {
