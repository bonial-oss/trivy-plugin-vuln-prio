...
// reportKnownFields lists the JSON keys that correspond to typed fields on
// Report. Everything else goes into Extras.
var reportKnownFields = map[string]bool{
	"SchemaVersion": true,
	"ArtifactName":  true,
	"ArtifactType":  true,
	"Metadata":      true,
...
// UnmarshalJSON decodes a Report from JSON, extracting known fields into
...
func (r *Report) UnmarshalJSON(data []byte) error {
	...
	if err := get("SchemaVersion", &r.SchemaVersion); err != nil {
		return err
	}
	if err := get("ArtifactName", &r.ArtifactName); err != nil {
		return err
	}
	if err := get("ArtifactType", &r.ArtifactType); err != nil {
		return err
	}
	if err := get("Results", &r.Results); err != nil {
		return err
	}

	if raw, ok := all["Metadata"]; ok {
		r.Metadata = raw
	}

	extras := make(map[string]json.RawMessage)
	for k, val := range all {
		if !reportKnownFields[k] {
			extras[k] = val
		}
	}
	if len(extras) > 0 {
...
// MarshalJSON encodes a Report to JSON, merging typed fields with the
...
func (r Report) MarshalJSON() ([]byte, error) {
	...
	m["SchemaVersion"] = r.SchemaVersion
	m["ArtifactName"] = r.ArtifactName
	m["ArtifactType"] = r.ArtifactType
	if r.Metadata != nil {
