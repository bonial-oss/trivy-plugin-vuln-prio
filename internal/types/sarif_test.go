...
func TestSARIFResult_EmptyExtras(t *testing.T) {
	...
	assert.NotContains(t, result, "ruleIndex")
	assert.Contains(t, result, "level")
	assert.Contains(t, result, "message")
}

func TestSARIFResult_RuleIndex_RoundTrip(t *testing.T) {
	t.Run("ruleIndex present", func(t *testing.T) {
		input := `{
			"ruleId": "CVE-2023-1234",
			"ruleIndex": 2,
			"level": "error",
		...
		// Verify ruleIndex was parsed.
		require.NotNil(t, r.RuleIndex, "RuleIndex should not be nil")
		assert.Equal(t, 2, *r.RuleIndex, "RuleIndex value")
		assert.Equal(t, "CVE-2023-1234", r.RuleID)

		// ruleIndex should not end up in Extras.
		assert.NotContains(t, r.Extras, "ruleIndex")
		...
		assert.Contains(t, result, "ruleId")
		assert.Contains(t, result, "ruleIndex")
		assert.Contains(t, result, "level")
		assert.Contains(t, result, "message")

		var idx int
		require.NoError(t, json.Unmarshal(result["ruleIndex"], &idx))
		assert.Equal(t, 2, idx, "ruleIndex round-trip value")
	})

	t.Run("ruleIndex zero", func(t *testing.T) {
		input := `{
			"ruleId": "",
			"ruleIndex": 0,
			"level": "warning",
			"message": {"text": "Index zero"}
		}`

		var r SARIFResult
		require.NoError(t, json.Unmarshal([]byte(input), &r))

		require.NotNil(t, r.RuleIndex, "RuleIndex should not be nil for explicit 0")
		assert.Equal(t, 0, *r.RuleIndex, "RuleIndex value")

		// Re-marshal and verify 0 is emitted.
		out, err := json.Marshal(r)
		require.NoError(t, err)
		...
		assert.Contains(t, result, "ruleIndex", "ruleIndex=0 should be present in output")

		var idx int
		require.NoError(t, json.Unmarshal(result["ruleIndex"], &idx))
		assert.Equal(t, 0, idx, "ruleIndex round-trip value for 0")
	})

	t.Run("ruleIndex absent", func(t *testing.T) {
		input := `{
			"ruleId": "CVE-2023-0001",
			"level": "warning",
			"message": {"text": "No ruleIndex"}
		}`

		var r SARIFResult
		require.NoError(t, json.Unmarshal([]byte(input), &r))

		assert.Nil(t, r.RuleIndex, "RuleIndex should be nil when absent")

		// Re-marshal and verify ruleIndex is not emitted.
		out, err := json.Marshal(r)
		require.NoError(t, err)
		...
		assert.NotContains(t, result, "ruleIndex", "ruleIndex should not be present when nil")
	})
}
