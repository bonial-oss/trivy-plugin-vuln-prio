...
Usage:
  ...
  trivy image -f sarif alpine:latest | trivy vuln-prio`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	flags.StringVar(&opts.Format, "format", "", "Output format override; only valid value: table")
...
func run(opts *Options) error {
	...
	if opts.Format == "table" && parsed.Format != input.FormatJSON {
		return &ExitError{
			Code:    3,
			Message: "--format table requires Trivy JSON input",
		}
	}
